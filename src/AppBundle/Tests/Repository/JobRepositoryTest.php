...
class JobRepositoryTest extends WebTestCase
{
    ...
    public function testCountActiveJobs()
    {
        ...
        foreach($categories as $category) {
            $query = $this->em->createQuery('SELECT COUNT(j.id) FROM AppBundle:Job j WHERE j.category = :category AND j.expiresAt > :date');
            $query->setParameter('category', $category->getId());
            $query->setParameter('date', date('Y-m-d H:i:s', time()));
            $jobs_db = $query->getSingleScalarResult();

            $jobs_rep = $this->em->getRepository('AppBundle:Job')->countActiveJobs($category->getId());
    ...
    public function testGetActiveJobs()
    {
        ...
        foreach ($categories as $category) {
            $query = $this->em->createQuery('SELECT COUNT(j.id) from AppBundle:Job j WHERE j.expiresAt > :date AND j.category = :category');
            $query->setParameter('date', date('Y-m-d H:i:s', time()));
            $query->setParameter('category', $category->getId());
            $jobs_db = $query->getSingleScalarResult();

            if($jobs_db > 2 ) {
                $jobs_rep = $this->em->getRepository('AppBundle:Job')->getActiveJobs($category->getId(), 2);
                ...
                $jobs_rep = $this->em->getRepository('AppBundle:Job')->getActiveJobs($category->getId(), 2, 1);
    ...
    public function testGetActiveJob()
    {
        ...
        $job_rep = $this->em->getRepository('AppBundle:Job')->getActiveJob($job_db->getId());
        ...
        $job_rep = $this->em->getRepository('AppBundle:Job')->getActiveJob($job_expired->getId());
